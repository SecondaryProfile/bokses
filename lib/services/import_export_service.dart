...
class ImportExportService {
  /// Export all boxes and items as JSON, then share/download the file.
  static Future<String?> exportData({Rect? sharePositionOrigin}) async {
    try {
      ...
      final itemMaps = await Future.wait(allItems.map((item) async {
        ...
        if (item.photoPath != null && item.photoPath!.isNotEmpty) {
          if (item.photoPath!.startsWith('data:')) {
            // Web: photo is already base64 — extract it directly
      ...
      if (kIsWeb) {
        // On web: return JSON string so the UI can trigger a download
        return jsonStr;
      } else {
        final dir = await getTemporaryDirectory();
        final timestamp =
            DateTime.now().toIso8601String().replaceAll(':', '-').substring(0, 19);
        final file = File('${dir.path}/bokses_export_$timestamp.json');
  ...
  /// Import boxes and items from a JSON file chosen by the user.
  ...
  static Future<String> importData() async {
    ...
    if (data['app'] != 'Bokses') {
      return 'Invalid Bokses export file.';
    }
    ...
    for (final bMap in boxMaps) {
      final box = Box.fromMap(Map<String, dynamic>.from(bMap));
